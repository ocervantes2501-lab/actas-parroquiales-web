...
@app.route("/agenda-sacerdotes", methods=["GET", "POST"])
def agenda_sacerdotes():
    """
    Página para que los sacerdotes de la parroquia consulten qué
    misas especiales hay programadas y se organicen entre ellos.
    Protegida con una clave simple compartida (NO es una cuenta de
    usuario) para que solo quien la conozca pueda entrar -- antes
    cualquiera con el enlace podía verla, lo cual exponía nombres y
    fechas de las familias.

    La clave se configura como variable de entorno CLAVE_SACERDOTES
    en Render. Si no está configurada, por seguridad esta página NO
    se muestra (mejor pedir que la configuren, que dejarla abierta).
    """
    clave_correcta = os.environ.get("CLAVE_SACERDOTES", "")
    if not clave_correcta:
        return render_template("agenda_sacerdotes.html", requiere_clave=True,
                                sin_configurar=True, error=None)

    if session.get("acceso_sacerdotes"):
        proximas = db.listar_reservaciones_futuras()
        return render_template(
            "agenda_sacerdotes.html", requiere_clave=False,
            proximas=proximas, etiquetas=db.ETIQUETAS_TIPO_RESERVACION,
        )

    if request.method == "POST":
        clave_escrita = request.form.get("clave", "")
        if clave_escrita == clave_correcta:
            session["acceso_sacerdotes"] = True
            return redirect(url_for("agenda_sacerdotes"))
        return render_template("agenda_sacerdotes.html", requiere_clave=True,
                                sin_configurar=False, error="Clave incorrecta.")

    return render_template("agenda_sacerdotes.html", requiere_clave=True,
                            sin_configurar=False, error=None)


@app.route("/consultar", methods=["GET", "POST"])
def consultar_reservacion():
    """
    Página pública (sin iniciar sesión) para que las familias
    consulten sus misas especiales agendadas (Exequias, XV Años,
    Boda, Aniversario, Otra). Solo muestra fecha, hora, tipo de
    misa y nombre -- nada de pagos ni datos sensibles.
    """
    resultados = None
    if request.method == "POST":
        telefono = request.form.get("telefono", "")
        nombre = request.form.get("nombre", "")
        resultados = db.buscar_reservaciones_publico(telefono, nombre)
    return render_template(
        "consultar.html", resultados=resultados, etiquetas=db.ETIQUETAS_TIPO_RESERVACION,
    )
...
def verificar(categoria, folio):
    ...
    elif categoria == "boleta":
        boleta = db.obtener_boleta_por_folio(folio)
        if boleta is None:
            return render_template("verificar.html", encontrado=False, categoria="Boleta")

        if not boleta.get("folio_acta_vinculado"):
            # Ya se emitió la boleta, pero todavía no se registra el
            # acta oficial en el libro -- se avisa claramente, sin
            # decir "auténtico" todavía.
            nombre = boleta["nombre"] if not boleta.get("nombre2") else f"{boleta['nombre']} & {boleta['nombre2']}"
            return render_template(
                "verificar.html", encontrado=True, categoria="Boleta", pendiente=True,
                datos={
                    "titulo": f"Boleta de {db.ETIQUETAS_TIPO_BOLETA.get(boleta['tipo'], boleta['tipo'])}",
                    "nombre": nombre,
                    "fecha": f"{boleta['dia_celebracion']} de {boleta['mes_celebracion']} de {boleta['anio_celebracion']}",
                    "folio": folio,
                },
            )

        # Ya está vinculada: se muestra como el acta ya registrada.
        registro = db.obtener_acta_por_folio(boleta["folio_acta_vinculado"])
        if registro is None:
            return render_template("verificar.html", encontrado=False, categoria="Boleta")
        datos = {
            "titulo": f"Acta de {registro.get('sacramento', '').title()}",
            ...
            "sacerdote": registro.get("sacerdote_sacramento", ""),
            "extra": f"Libro {registro.get('num_libro','')} · Foja {registro.get('num_foja','')} · Acta {registro.get('num_acta','')}",
            "folio": boleta["folio_acta_vinculado"],
        }
        return render_template("verificar.html", encontrado=True, categoria="Acta", datos=datos)
...
def agenda():
    if request.method == "POST":
        ...
        datos = dict(
            ...
            sacerdote_celebrante=request.form.get("sacerdote_celebrante", "").strip(),
            precio_total=precio,
            monto_pagado=0,
            estatus="PENDIENTE",
        )
