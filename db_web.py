...
def buscar_reservaciones_publico(telefono: str, nombre: str = ""):
    """
    Búsqueda pública (sin necesidad de iniciar sesión) para que las
    familias consulten SU PROPIA misa especial. Por privacidad, exige
    el teléfono EXACTO con el que se registró la reservación (no
    basta con adivinar el nombre); el nombre es un filtro adicional
    opcional. No incluye ningún dato de pago.
    """
    telefono_limpio = "".join(c for c in (telefono or "") if c.isdigit())
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT {','.join(COLUMNAS_RESERVACION)} FROM reservaciones "
        f"ORDER BY fecha, hora"
    )
    filas = cur.fetchall()
    conn.close()
    registros = [dict(zip(COLUMNAS_RESERVACION, f)) for f in filas]
    ...
    resultado = [
        r for r in registros
        if "".join(c for c in str(r.get("telefono") or "") if c.isdigit()) == telefono_limpio
    ]
    if nombre.strip():
        texto = nombre.strip().upper()
        resultado = [r for r in resultado if texto in str(r.get("nombre") or "").upper()]
    return resultado
...
def listar_reservaciones_futuras():
    """
    Todas las misas especiales (Exequias, XV Años, Boda, Aniversario,
    Otra) desde hoy en adelante, ordenadas por fecha -- para que los
    sacerdotes puedan organizarse entre ellos. Incluye lugar/capilla,
    quién la celebra, y la descripción (si la tiene). NO incluye
    teléfono ni datos de pago.
    """
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT fecha, hora, tipo, nombre, lugar, sacerdote_celebrante, descripcion "
        "FROM reservaciones WHERE fecha >= ? ORDER BY fecha, hora",
        (hoy,),
    )
    filas = cur.fetchall()
    conn.close()
    return [
        {
            "fecha": f[0], "hora": f[1], "tipo": f[2], "nombre": f[3],
            "lugar": f[4], "sacerdote_celebrante": f[5], "descripcion": f[6],
        }
        for f in filas
    ]
...
# =====================================================
# Boletas (para el código QR "pendiente de registro")
# =====================================================
COLUMNAS_BOLETA = [
    "folio", "tipo", "nombre", "nombre2",
    "dia_celebracion", "mes_celebracion", "anio_celebracion", "sacerdote_celebro",
    "dia_nacimiento", "mes_nacimiento", "anio_nacimiento", "lugar_nacimiento",
    "padres", "abuelos_paternos", "abuelos_maternos", "padrinos", "testigos",
    "rc_numero", "rc_fecha", "rc_lugar",
    "sacerdote_firma", "cargo", "fecha_emision", "folio_acta_vinculado",
]

ETIQUETAS_TIPO_BOLETA = {"BAUTIZO": "Bautizo", "MATRIMONIO": "Matrimonio"}


def obtener_boleta_por_folio(folio: int):
    conn = conectar()
    cur = conn.cursor()
    cur.execute(f"SELECT {','.join(COLUMNAS_BOLETA)} FROM boletas WHERE folio = ?", (folio,))
    fila = cur.fetchone()
    conn.close()
    return dict(zip(COLUMNAS_BOLETA, fila)) if fila else None
