...
def listar_reservaciones_futuras():
    ...
    sacerdotes puedan organizarse entre ellos. No incluye teléfono
    ni datos de pago (solo lo necesario para saber qué hay agendado).
    """
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT fecha, hora, tipo, nombre FROM reservaciones "
        "WHERE fecha >= ? ORDER BY fecha, hora",
        (hoy,),
    )
    filas = cur.fetchall()
    conn.close()
    return [{"fecha": f[0], "hora": f[1], "tipo": f[2], "nombre": f[3]} for f in filas]
