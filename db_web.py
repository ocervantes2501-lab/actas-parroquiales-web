...
def verificar_login(usuario: str, password: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT usuario, password_hash, salt, es_admin, activo FROM usuarios WHERE usuario = ?",
        (usuario.strip(),),
    )
    fila = cur.fetchone()
    conn.close()
    if fila is None:
        return None
    _, password_hash, salt, es_admin, activo = fila
    if not activo:
        return None
    ...
    if not hmac.compare_digest(intento, password_hash):
        return None
    return {"usuario": usuario.strip(), "es_admin": bool(es_admin)}
